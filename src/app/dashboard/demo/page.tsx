...
import { stat } from 'fs';
  ...
  export default function DropzoneGame() {
    ...
    const [controlPoints, setControlPoints] = useState<ControlPoint[]>([
      ...
      { id: 'cp-40', position: { x: 5200, y: 5800 }, radius: 60, ownedByPlayer: false, color: '#ff4444', progress: 0 }
    ]);
...
useEffect(() => {
  const enemies: Unit[] = [
    { 
      ...
      name: 'Enemy Troop', 
      category: 'enemy', 
      imagePath: '../images/enemies/enemy_default.png',
  ...
  setUnits(enemies);
    ...
    }, [controlPoints]); // Only depends on controlPoints
    ...
    // Game loop - FIXED with useRef to prevent infinite loops
    const gameLoopRef = useRef<NodeJS.Timeout>();

    // Update the game loop to properly destroy units
    useEffect(() => {
      ...
      gameLoopRef.current = setInterval(() => {
        setUnits(prevUnits => {
          ...
          // Filter out destroyed units (health <= 0)
          const aliveUnits = updatedUnits.filter(unit => {
            if (unit.health <= 0) {
              ...
              return false; // Remove destroyed unit
            }
            return true; // Keep alive unit
          });
          
          if (unitsChanged) {
            return aliveUnits; // Return filtered array if units were destroyed
          }
          ...
          aliveUnits.forEach(unit => {
            ...
            if (unit.carriedBy) {
              ...
              if (transport && transport.health > 0) {
                // Unit should exactly match transport position when carried
    ...
    }, [mapDimensions, canUnitMoveThroughPosition, transports]);

    // Control point capture system - FIXED
    ...
    setUnits(currentUnits => {
      setControlPoints(prevCPs => {
        return prevCPs.map(cp => {
          ...
          if (playerUnitsInRange > 0 && !cp.ownedByPlayer) {
            const newProgress = Math.min(100, cp.progress + 10);
            return { 
              ...cp, 
              progress: newProgress,
              ownedByPlayer: newProgress >= 100,
              color: newProgress >= 100 ? '#44ff44' : '#ffaa00'
            };
          } else if (playerUnitsInRange === 0 && !cp.ownedByPlayer && cp.progress > 0) {
  ...
  return () => {
    ...
    }, [showTransportUI, viewport, mapDimensions, draggingUnitType, unitTemplates, canPlaceUnitAtPosition, isInConqueredArea, economy.money, unitCost, canUnitMoveThroughPosition, units]);
  ...
  if (!clickedUnit?.isTransport || !clickedUnit.selected) {
  ...
  }, []); // Empty dependency array - doesn't depend on any state
    ...
    // Transport functions (keep as is)
    ...
    // Unload a specific unit - FIXED to unload at transport position
    const handleUnloadUnit = (unitId: string) => {
      ...
      setUnits(prev => prev.map(unit => {
        if (unit.id === unitId && unit.carriedBy === selectedTransport) {
          const transport = prev.find(t => t.id === selectedTransport);
          if (transport && transport.health > 0) {
            // Unload at the transport's exact position (0,0 offset)
            return {
              ...unit,
              carriedBy: undefined,
              loading: false,
              unloading: false,
              position: {
                x: transport.position.x, // Exact same X
                y: transport.position.y  // Exact same Y
              }
            };
          }
        }
    ...
    // Unload all units - FIXED to unload at transport position
      ...
      setUnits(prev => {
        ...
        return prev.map(unit => {
          if (unit.carriedBy === selectedTransport) {
            // Unload all at the transport's exact position
            return {
              ...unit,
              carriedBy: undefined,
              loading: false,
